...
/*   Updated: 2025/08/24 16:33:52 by kyamanak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	say_0_999(t_dict *d, int n, int *need_sp)
{
	...
	// 	return (0);
	if (!put_word(d, k, need_sp)) {
    printf("DEBUG: put_word failed for key=%s\n", k);
    return (0);
}
if (!put_word(d, "100", need_sp)) {
    printf("DEBUG: put_word failed for key=100\n");
    return (0);
}
...
int put_word(t_dict *dict, char *key, int *need_sp)
{
    char *word = find_in_dict(dict, key);
    if (!word) {
        printf("\nDEBUG put_word failed: key=%s not found\n", key);
        return 0;
    }
    if (*need_sp)
        printf(" ");
